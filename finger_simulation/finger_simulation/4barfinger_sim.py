...
def main():
    ...
    # Load finger SDF
    model_file_url = "package://finger_description/sdf/finger.sdf"
    ...
    p_AP = numpy.array([0.0001,  0.0348, -0.0147])  # dip_flex2 in middle_phalanx2 frame
    p_BQ = numpy.array([-0.0054, 0.0022, -0.0087])  # dip_flex2 in distal_phalanx frame

    plant.AddCouplerConstraint(
        joint_A=plant.GetJointByName("pip_flex1", finger),
        joint_B=plant.GetJointByName("dip_flex1", finger),
        gear_ratio=0.85,   # tune this to match your four-bar geometry
        offset=0.0,
    )
